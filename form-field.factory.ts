...
abstract class FormFieldFactory {
  ...
  protected findFormFieldElement(formField: FormField): any {
  ...
  private getLeafElementByElementType(
    ...
    switch (elementType) {
      ...
      case FORM_FIELD_ELEMENT_TYPE.DROPDOWN:
        return "dropdown";
      default:
        throw new Error(`Unexpected form field element type: ${elementType}`);
    }
  }
}
...
export class TextFormFieldFactory extends FormFieldFactory {
  ...
  createFormField(): Setting {
    ...
    setting.addText((text) =>
      text.setValue(this.resolveFormFieldValue()).onChange((value) => {
        new Notice(`${this.formField.className} changed: ${value}`);

        this.dependentFields?.map((dependentField): void => {
          const dependentFieldEl = this.findFormFieldElement(dependentField);
          if (dependentFieldEl)
            (dependentFieldEl as HTMLInputElement).value = value;

          dependentField.content["value"] = value;
        });
    ...
    if (this.formField.description) setting.setDesc(this.formField.description);
...
export class DropownFormFieldFactory extends FormFieldFactory {
  ...
  createFormField(): Setting {
    ...
    setting.addDropdown((dropdown) =>
      dropdown.addOptions(this.formField.options).onChange((value) => {
        new Notice(`${this.formField.className} changed: ${value}`);
...
export class Form {
  ...
  private getDependentFields(fieldClassName: string): FormField[] | undefined {
